...
def findBasinLine(line, position):
    ...
    while (
        position_left >= 0
        and line[position_left] > line[position_left + 1]
        and line[position_left] != 9
    ):
    ...
    while (
        position_right < len(line)
        and line[position_right] > line[position_right - 1]
        and line[position_right] != 9
    ):
...
def main():
    with open(path, "r") as file:
        ...
        for i in range(0, len(lines)):
            heightmap[i] = np.array([char for char in lines[i].replace("\n", "")])
        ...
        for i in range(0, len(heightmap)):
            for j in range(0, len(heightmap[0])):
                ...
                if not (
                    i == len(heightmap) - 1 or heightmap[i][j] < heightmap[i + 1][j]
                ):
                    isLocalMinimum = False
                if not (j == 0 or heightmap[i][j] < heightmap[i][j - 1]):
                    isLocalMinimum = False
                if not (
                    j == len(heightmap[0]) - 1 or heightmap[i][j] < heightmap[i][j + 1]
                ):
                    isLocalMinimum = False
                if isLocalMinimum:
        ...
        for i in range(0, len(heightmap)):
            for j in range(0, len(heightmap[0])):
                if heightmap[i][j] != -2:
                    ...
                    if not (
                        j == len(heightmap[0]) - 1
                        or heightmap[i][j] < heightmap[i][j + 1]
                    ):
                        isLocalMinimum = False
                    if isLocalMinimum:
                        ...
                        while worked:
                            ...
                            for k in range(0, len(heightmap)):
                                for l in range(0, len(heightmap[0])):
                                    if heightmap[k][l] == -1:
                                        result_x = findBasinLine(heightmap[k, :], l)
                                        if result_x[1]:
                                            # we found a connected field
                                            worked = True
                                            heightmap[k] = result_x[0]
                                        result_y = findBasinLine(heightmap[:, l], k)
                                        if result_y[1]:
                                            # we found a connected field
                                            worked = True
